...
static char *substr(char *str, int offset, int limit) {
  ...
  if ((limit > str_size) || ((offset + limit) > str_size) || (str_size < 1) ||
      (limit == 0))
    return NULL;
...
static int transposition(char *word, char **array, int start_idx) {
  ...
  for (i = 0; i < word_len - 1; i++)
    array[i + start_idx] = concat(
        concat(substr(word, 0, i), substr(word, i + 1, 1)),
